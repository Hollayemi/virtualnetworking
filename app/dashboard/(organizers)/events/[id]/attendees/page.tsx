...
import { 
  ...
  MessageCircle, Link2, ExternalLink, ChevronLeft, ChevronRight as ChevronRightIcon
} from 'lucide-react';
...
import { useGetEventDetailQuery, useGetEventAttendeesQuery } from '@/redux/slices/organiser/events.slice';
...
export default function AttendeesPage() {
  ...
  const { data: eventData } = useGetEventDetailQuery(eventFilterId as string, { skip: !eventFilterId });
  const eventFilter = eventData?.data as any;
    ...
    <>
      ...
      >
        <div>
          ...
          <p className="mt-1 text-[13.5px] text-[#92A79C]">Everyone registered across your events — click into anyone for the full profile.</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <StatCard icon={Users} label="VIP attendees" value={String(vipCount)} accent={accent} />
        </div>

        {eventFilterId && (
          <div className="flex items-center justify-between rounded-xl border border-[#D9B26B]/25 bg-[#151009]/40 px-4 py-2.5">
            <span className="text-[12.5px] text-[#E3C08A]">
              Filtered to <span className="font-medium">{eventFilter?.name ?? 'selected event'}</span>
            </span>
            <Link href="/dashboard/attendees" className="flex items-center gap-1 text-[12px] text-[#92A79C] hover:text-[#EAF2ED]">
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="truncate text-[12px] text-[#7C8F85]">
                          {user.bio || 'No bio'} · {attendee.tierLabel}
                        </p>
                      </div>
                      <span className="hidden text-[12px] text-[#7C8F85] sm:block">
